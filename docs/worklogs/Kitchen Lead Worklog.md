...
| DEC-043 | **The two-view screen is the game's screen.** `project.godot`'s `run/main_scene` now points at it; `kitchen_screen.tscn` stops being what launches. This is the Week 3 lock GDD v9 added as an exit item, and it unblocks #43's art sourcing, which could not choose assets without knowing what slots they fill. The lock followed the worktop rebuild (#44) rather than the two-view structure alone -- the owner's verdict on the stacked-bands version was "just an abstraction", and on the stations version "it does read as a workspace". `kitchen_screen.gd` is **not** deleted here: it still runs, its tests still pass, and retiring it is a separate change carrying its own risk. The file is still named `greybox`, which is now wrong, and that is tracked rather than quietly tolerated. | Approved | Human |
| DEC-044 | **The ticket is a reminder, not a replacement, and the five-second recall test is withdrawn.** The brief made per-view five-second recall the load-bearing exit test and called the preparation half "the direct test of whether the ticket carries the request into preparation, which is the whole claim of the two-view design". The owner rejected the premise after playing it: *"Why do we need the speed? In fact I'm okay going back and forth between the customer screen and see their full request and the prep screen."* The deeper objection is the one that changes the design rather than the test -- **not everything a customer says can be condensed**. A standing love of noodles is not a flavour target and does not fit on a docket, so a ticket that passed a strict recall test would be one that had thrown that away. Consequence, and the reason this is a build change and not a doc edit: the screen had **no way back** to the request once confirmed. It has one now, and the tray survives the trip -- a player who loses two chosen ingredients by looking something up will stop looking things up. Replaces the recall test as the exit criterion: moving between the views costs the player nothing they have already chosen. | Approved | Human |
| DEC-045 | **Every surface carries a border, and a ground is never painted the token its panels use.** Rule 2's alpha floor guarantees the *text* on a panel is readable and says nothing about whether the panel's **edge** is findable. The first render of the worktop proved those are different questions: `surface` panels on a `surface` ground, and blocks falling through to Godot's default button stylebox because this theme had never defined `Button/styles/normal` at all -- so no station, ticket, or pass boundary was visible anywhere on screen, and the owner reported the blocks "blend in with the background." Panels take a 1px `disabled_surface` border; buttons a 2px `text_muted` border over an **opaque** `surface` fill; hover and focus raise it to `accent`, focus at 3px rather than a different hue, so a keyboard user and a mouse user are not told two unrelated stories about one state. Corner radius 4 throughout. **No new colour** -- rule 3 is untouched, these are weights and radii drawn from the eight ratified tokens. The paired ground rule is what stops it recurring: `background` for the worktop, `surface` for what stands on it. | Approved | Human |
| DEC-046 | **Delete `kitchen_screen.gd` and drop the greybox name** (#45). The old panel screen no longer launched after DEC-043, and a passing 17-test suite describing an interface no player would ever see is worse than no suite: it inflates the count, resists every refactor, and asserts behaviour nobody has agreed to keep. Git keeps it. The two-view screen moves out of `greybox/` and takes the `KitchenScreen` / `IngredientBlock` names, because a shipping file called `greybox_*` behind a doc comment reading "A time-boxed, disposable greybox" is documentation that is actively wrong rather than merely stale. Deliberate consequence: the suite drops 245 -> 228 tests and 27 -> 26 scripts, and the new floor in `check.sh` is lowered in the same commit, which is the intended way to spend it. | Approved | Human |
| DEC-047 | **Stations are shelves: they hold what fits, wrap, and scroll; their zones never grow.** The four zones were sized around the current 2/2/3/5 split, which is a fact about today's pantry rather than a property of the design -- and #24 triples it without saying how it lands. A station that expanded to fit would overrun the pass and its neighbours. Surfaced in review: the test claiming the pantry scaled to 6/12/24 built mock blocks in a *scratch* container and never touched the worktop, so it could not have failed if a station clipped everything it was given. **The distribution is the risk, not the total** -- replaced with tests that restock the real stations at 20 in the narrowest and an uneven 1/1/2/20. Also collapses the is-this-a-column flag: every station now uses the same `ScrollContainer` + `HFlowContainer`, because at 0.15 of the width a 124px block wraps one per row on its own, which is all the flag ever produced. | Approved | Human |
| DEC-048 | **The interaction floor is enforced on `IngredientBlock`, not asserted about it.** My earlier reasoning -- leave it unclamped so the test can fail -- was backwards, and the review said so: a guard is falsified by removing the guard, not by never having one, and a guarantee nothing enforces is a comment. The clamp lives on the block rather than its caller because the thing that has to be hittable is the right place for a promise about hittability. The old test was also weaker than it looked: it asserted `custom_minimum_size`, the size a block *requests*, so a station squeezing one below the floor would have passed. Three tests now, because one assertion cannot do all three jobs -- a too-small silhouette is raised, a legal one passes through unflattened (a clamp that squared every block would satisfy the hit target and destroy the variation it protects), and every block's **rendered** size clears the floor after layout. | Approved | Human |
| DEC-049 | **Two spatial views, four presentation states, and the enum is `ScreenState`.** Calling it `View` conflated a container with a state and made the model read as four screens; `RESULT` and `ENDED` both render through `CustomerView`, because a result belongs to whoever ordered it and the night ends with the last of them. Renamed in code as well as in the document -- a doc and an implementation disagreeing about the central noun is exactly the drift DEC-046 had just been spent clearing. | Approved | Human |
| DEC-050 | **The screen reads three sources under three rules, not one.** "Renders only from `DomainEvent`/`CommandResult`" was too absolute and contradicted its own text: the screen reads `state()` and `content()`, and must. Authored content is free to read; session state is read-only, held by reference rather than copied so rendering cannot go stale against it; **evaluated facts come only from events** -- score, band, strongest match, largest miss, constraint outcome, reaction, never recomputed and never derived. The third is the rule worth defending and the only one source-scanned. The distinction is practical rather than pedantic: reading `current_dish` to render the pass is correct, and reading `dish_profile` to render how savoury it is would be the same call shape and a design violation. | Approved | Human |
...
|---|---|---|
| Q-008 | **What represents the customer?** Portrait bust, silhouette, full figure, hands, or no figure. Art Asset Brief §9; never answered. | The entire `REQUEST` view art round. Five options, five non-overlapping searches |
| Q-009 | **`kimchi` — search again, or accept the gap?** Heat & ferment needs a third distinct squat form and only two exist. Recolouring a fourth would breach Visual Language rule 1. | Finishing the ingredient set |
...
| `docs/design/asset-coverage-probe.md` | AS-02 — coverage, Pass A, text only | Reference |
| `docs/design/asset-coverage-verified.md` | AS-03 — coverage, Pass B, by sight | Reference |
| `docs/design/asset-icon-shortlist.md` | The chosen twelve, fill states, and how to refetch them | Active |
