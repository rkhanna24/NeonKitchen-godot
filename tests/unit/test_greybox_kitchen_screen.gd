## `GreyboxKitchenScreen` (docs/design/references/godot-greybox-ideas.md).
##
...
const SCENE_PATH: String = "res://adapters/godot_ui/greybox/greybox_kitchen_screen.tscn"
...
func test_pressing_an_ingredient_marks_it_and_fills_the_tray() -> void:
	_screen._confirm_button.pressed.emit()
	var block: GreyboxIngredientBlock = _find_block(&"ingredient.mushrooms")
...
func test_focusing_an_ingredient_by_keyboard_shows_its_full_description() -> void:
	_screen._confirm_button.pressed.emit()
	var block: GreyboxIngredientBlock = _find_block(&"ingredient.coconut_milk")
...
## GDD section 5.1, the mechanical rule `test_kitchen_screen.gd` enforces for
## `KitchenScreen`. Repeated here rather than assumed, because that suite
## only scans `res://adapters/godot_ui` itself, not the `greybox/`
## subdirectory `DirAccess.get_files()` does not recurse into.
func test_the_greybox_contains_no_scoring_or_constraint_logic() -> void:
	var forbidden: Array[String] = ["Evaluator", "FlavourScorer", "ConstraintChecker"]
	var dir := DirAccess.open("res://adapters/godot_ui/greybox")
	assert_not_null(dir, "the greybox directory exists")
	...
	for file_name: String in dir.get_files():
		...
		var source: String = FileAccess.get_file_as_string(
			"res://adapters/godot_ui/greybox/%s" % file_name
		)
...
## No `add_theme_*_override` call anywhere in the greybox -- the same rule
## `test_kitchen_screen.gd` enforces for `KitchenScreen`, repeated here for
## the same reason as the check above.
func test_no_theme_property_is_set_inline_in_the_greybox() -> void:
