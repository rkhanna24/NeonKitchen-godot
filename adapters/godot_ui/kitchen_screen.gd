## The game's screen: one encounter across two player-facing views inside the
## same food truck, a request view and a preparation view.
##
## Designed from the human's own reference research
## (docs/design/references/frame-notes-and-greybox-brief.md and
## godot-greybox-ideas.md) and locked as the shipping interface in DEC-043.
##
## It began as a disposable greybox beside an earlier panel-based screen. That
## screen has been deleted (#45): it no longer launched, and a passing suite
## describing an interface no player would see was worse than no suite at all.
...
var _ingredient_blocks: Array[IngredientBlock] = []
...
func _populate_pantry() -> void:
	...
	for group: StringName in IngredientDefinition.GROUPS:
		...
		for ingredient: IngredientDefinition in by_group[group]:
			var block: IngredientBlock = _ingredient_block(ingredient)
...
func _link_focus_chain() -> void:
	...
	for index: int in range(_ingredient_blocks.size()):
		var block: IngredientBlock = _ingredient_blocks[index]
		if index < last:
