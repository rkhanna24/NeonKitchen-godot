...
## Seven recognised verbs, plus `INVALID` for anything `parse()` could not
## resolve into one of the other seven. `QUIT` and `LIST` are adapter-only:
## neither is ever sent to `CommandHandler`, so neither is one of ADR 0004
## section 7's five commands. `LIST` is a *query* — it reads content and
## session state, changes nothing, and emits no event, which is precisely why
## it must not travel through the command path.
enum Verb { START, PRESENT, SELECT, REMOVE, SUBMIT, QUIT, LIST, INVALID }
...
static func parse(line: String) -> ParsedLine:
	...
	match verb_token:
		...
		"list":
			return _no_argument(Verb.LIST, rest, verb_token)
		"select":
		...
		_:
			return ParsedLine.new(
				...
				(
					(
						"unknown command '%s' -- try start, present, list, "
						+ "select <id>, remove <id>, submit, or quit"
					)
					% verb_token
				)
			)
